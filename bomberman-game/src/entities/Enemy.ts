...
export class Enemy {
  ...
  private createVisuals(): void {
    ...
    // Body shape varies by type - scaled up 3x for much better visibility
    if (this.type === 'balloon') {
      ...
      // String
      const stringGeo = new THREE.CylinderGeometry(0.06, 0.06, 0.6);
      ...
      // Onion - layered, onion-like - scaled up 3x
      const bodyGeo = new THREE.ConeGeometry(0.9, 1.8, 16);
      const bodyMat = new THREE.MeshStandardMaterial({ color });
      this.bodyMesh = new THREE.Mesh(bodyGeo, bodyMat);
      ...
      // Layers detail
      const ringGeo = new THREE.TorusGeometry(0.75, 0.09, 8, 16);
      ...
      // Tiger - sleek, angular - scaled up 3x
      const bodyGeo = new THREE.BoxGeometry(1.2, 1.5, 1.8);
      const bodyMat = new THREE.MeshStandardMaterial({ color });
      this.bodyMesh = new THREE.Mesh(bodyGeo, bodyMat);
      this.bodyMesh.position.y = 1.05;
      this.bodyMesh.castShadow = true;
      ...
      // Stripes
      const stripeGeo = new THREE.BoxGeometry(1.26, 0.3, 1.2);
    ...
    // Eyes (common to all) - scaled up 3x
    ...
    this.eyeLeft.position.set(-0.3, 1.65, 0.6);
    this.mesh.add(this.eyeLeft);
    
    this.eyeRight = new THREE.Mesh(eyeGeo, eyeMat);
    this.eyeRight.position.set(0.3, 1.65, 0.6);
