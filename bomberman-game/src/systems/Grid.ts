...
import { GRID_WIDTH, GRID_HEIGHT, CELL_SIZE, LEVEL_THEMES, type LevelTheme, type PowerUpType, POWERUP_DROP_CHANCE } from '../utils/Constants';
...
export class Grid {
  ...
  private theme: LevelTheme = LEVEL_THEMES.grasslands;
  ...
  clearSpawnArea(cx: number, cy: number): void {
  ...
  destroyWall(x: number, y: number): PowerUpType | null {
    const cell = this.getCell(x, y);
    if (!cell || !cell.hasDestructibleWall) return null;
    cell.hasDestructibleWall = false;
    if (cell.wallMesh) {
  ...
  private spawnPowerUpMesh(cell: Cell): void {
  ...
  collectPowerUp(x: number, y: number): PowerUpType | null {
    const cell = this.getCell(x, y);
    if (!cell || !cell.powerUp) return null;
