import { GameState, TileType, GRID_SIZE, GridPos, Direction, Vec2 } from '@core/types';

export function isWalkable(grid: TileType[][], pos: GridPos): boolean {
  if (pos.col < 0 || pos.col >= GRID_SIZE || pos.row < 0 || pos.row >= GRID_SIZE) return false;
  return grid[pos.row][pos.col] === TileType.Floor;
}

export function gridToWorld(pos: GridPos): Vec2 {
  return { x: pos.col, y: pos.row };
}

export function worldToGrid(pos: Vec2): GridPos {
...
export function neighborPos(pos: GridPos, dir: Direction): GridPos {
  const v = directionToVec(dir);
  return { col: pos.col + v.x, row: pos.row + v.y };
}

export function movePlayer(state: GameState, playerId: number, dt: number): void {
  const player = state.players[playerId];
  if (!player || !player.alive || player.moveDir === Direction.None) return;
