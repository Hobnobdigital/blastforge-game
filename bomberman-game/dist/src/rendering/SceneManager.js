...
import { TileType, GRID_SIZE, TILE_WORLD_SIZE } from '@core/types';
...
export class SceneManager {
    ...
    powerUpMeshes = [];
    // Materials
    matFloor = new THREE.MeshStandardMaterial({ color: 0x2a2a2a });
    matHard = new THREE.MeshStandardMaterial({ color: 0x555555 });
    matSoft = new THREE.MeshStandardMaterial({ color: 0x8b6914 });
    matPlayer = new THREE.MeshStandardMaterial({ color: 0x00aaff });
    matBomb = new THREE.MeshStandardMaterial({ color: 0x111111 });
    matBombPrimed = new THREE.MeshStandardMaterial({ color: 0x3344ff, emissive: 0x1122aa });
    matBombRushed = new THREE.MeshStandardMaterial({ color: 0xff2222, emissive: 0xaa1111 });
    matExplosion = new THREE.MeshStandardMaterial({ color: 0xff6600, emissive: 0xff4400, transparent: true, opacity: 0.85 });
    matPowerUp = new THREE.MeshStandardMaterial({ color: 0x44ff44, emissive: 0x22aa22 });
    // Shared geometries
    geoBlock = new THREE.BoxGeometry(TILE_WORLD_SIZE * 0.95, TILE_WORLD_SIZE * 0.95, TILE_WORLD_SIZE * 0.95);
    geoPlayer = new THREE.BoxGeometry(0.7, 0.9, 0.7);
    geoBomb = new THREE.SphereGeometry(0.35, 16, 16);
    geoExplosion = new THREE.BoxGeometry(TILE_WORLD_SIZE * 0.9, 0.3, TILE_WORLD_SIZE * 0.9);
    geoPowerUp = new THREE.BoxGeometry(0.4, 0.4, 0.4);
    dummy = new THREE.Object3D();
    // Weather system
    weatherSystem;
    currentTheme = LevelTheme.CLASSIC;
    constructor() {
    ...
    syncExplosions(state) {
        ...
        for (let i = 0; i < state.explosions.length; i++) {
            ...
            mesh.material.opacity = Math.min(1, e.remaining * 2);
        }
    }
    syncPowerUps(state) {
