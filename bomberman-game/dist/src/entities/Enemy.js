...
export class Enemy {
    ...
    update(dt, grid, playerGridX, playerGridY, bombPositions) {
        if (!this.alive)
            return;
    ...
    updateTigerAI(grid, playerX, playerY, bombPositions) {
    ...
    canMove(wx, wz, r, grid, bombPositions = []) {
        const corners = [
            { x: wx - r, z: wz - r },
            { x: wx + r, z: wz - r },
            { x: wx - r, z: wz + r },
    ...
    canMoveInDirection(dir, grid) {
        const nextX = this.gridX + dir.x;
        const nextY = this.gridY + dir.y;
        const cell = grid.getCell(nextX, nextY);
        return cell !== null && cell.isWalkable();
    }
    pickNewDirection(grid) {
        const directions = [
            { x: 0, y: -1 },
            { x: 0, y: 1 },
            { x: -1, y: 0 },
        ...
        if (valid.length > 0) {
            ...
            const nonReverse = valid.filter(dir => !(dir.x === -this.direction.x && dir.y === -this.direction.y));
