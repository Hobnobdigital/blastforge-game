...
export function placeBomb(state, playerId) {
    const player = state.players[playerId];
    if (!player || !player.alive)
        return false;
    if (player.activeBombs >= player.maxBombs)
        return false;
    if (state.bombs.some(b => b.gridPos.col === player.gridPos.col && b.gridPos.row === player.gridPos.row))
        return false;
    const bomb = {
        id: nextBombId++,
...
export function tickBombs(state, dt) {
    const detonated = [];
...
export function primeBomb(state, playerId) {
    const player = state.players[playerId];
    if (!player || player.fuseCharges <= 0)
        return false;
...
export function rushBomb(state, playerId) {
    const player = state.players[playerId];
    if (!player || player.fuseCharges <= 0)
        return false;
...
export function detonateBomb(state, playerId) {
    const player = state.players[playerId];
    if (!player || player.fuseCharges <= 0)
        return null;
