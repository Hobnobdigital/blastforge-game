...
(function (WeatherType) {
    ...
    WeatherType["ASH"] = "ash";
    WeatherType["POLLEN"] = "pollen";
    WeatherType["NEON"] = "neon";
...
export class WeatherSystem {
    ...
    snowAccumulation = new Map();
    accumulationMesh;
    accumulationDummy = new THREE.Object3D();
    // Bounds
    bounds = { minX: -2, maxX: 18, minZ: -2, maxZ: 18, height: 20 };
    ...
    initializeParticleMesh() {
        ...
        this.particleMesh = new THREE.InstancedMesh(this.rainGeo, this.rainMaterial, this.particleCount);
        this.particleMesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
        this.particleMesh.count = 0; // Start hidden
        this.scene.add(this.particleMesh);
    }
    initializeSplashes() {
        this.splashMesh = new THREE.InstancedMesh(this.splashGeo, new THREE.MeshBasicMaterial({
            color: 0x44aaff,
            transparent: true,
            opacity: 0.5,
            blending: THREE.AdditiveBlending,
    ...
    initializeAccumulation() {
        ...
        this.accumulationMesh = new THREE.InstancedMesh(new THREE.CylinderGeometry(0.4, 0.5, 0.1, 6), new THREE.MeshStandardMaterial({
            color: 0xffffff,
            transparent: true,
            opacity: 0.6,
            roughness: 0.8,
    ...
    initializeFireGlow() {
        ...
        this.fireGlowMesh = new THREE.Mesh(glowGeo, new THREE.MeshBasicMaterial({
            color: 0xff2200,
            transparent: true,
            opacity: 0,
            blending: THREE.AdditiveBlending,
    ...
    setWeather(weather, intensity = 0.5) {
        if (this.currentWeather === weather && this.intensity === intensity)
            return;
    ...
    updateWeatherGeometry() {
        ...
        switch (this.currentWeather) {
            ...
            case WeatherType.SNOW:
                this.particleMesh = new THREE.InstancedMesh(this.snowGeo, this.snowMaterial, this.particleCount);
                break;
            case WeatherType.ASH:
                this.particleMesh = new THREE.InstancedMesh(this.ashGeo, this.ashMaterial, this.particleCount);
                break;
            case WeatherType.POLLEN:
                this.particleMesh = new THREE.InstancedMesh(this.pollenGeo, this.pollenMaterial, this.particleCount);
                break;
            case WeatherType.NEON:
                this.particleMesh = new THREE.InstancedMesh(this.neonGeo, this.neonMaterial, this.particleCount);
                break;
            default:
                this.particleMesh = new THREE.InstancedMesh(this.rainGeo, this.rainMaterial, this.particleCount);
        }
    ...
    updateFireGlow() {
        if (this.currentWeather === WeatherType.ASH) {
            ...
            this.fireGlowMesh.material.opacity = 0.15 * this.intensity;
        }
        else {
            this.fireLight.intensity = 0;
            this.fireGlowMesh.material.opacity = 0;
    ...
    spawnParticle(particle) {
        ...
        switch (this.currentWeather) {
            case WeatherType.RAIN:
                particle.velocity.set((Math.random() - 0.5) * 2, -15 - Math.random() * 5, (Math.random() - 0.5) * 2);
                break;
            case WeatherType.SNOW:
                particle.velocity.set((Math.random() - 0.5) * 1, -2 - Math.random() * 2, (Math.random() - 0.5) * 1);
                break;
            case WeatherType.ASH:
                particle.velocity.set((Math.random() - 0.5) * 3, 2 + Math.random() * 3, (Math.random() - 0.5) * 3);
                break;
            case WeatherType.POLLEN:
                particle.velocity.set((Math.random() - 0.5) * 0.5, -0.5 - Math.random() * 0.5, (Math.random() - 0.5) * 0.5);
                break;
            case WeatherType.NEON:
                particle.velocity.set((Math.random() - 0.5) * 1, 3 + Math.random() * 4, (Math.random() - 0.5) * 1);
                break;
        }
    }
    spawnSplash(x, z) {
    ...
    update(dt) {
        if (!this.enabled || this.currentWeather === WeatherType.NONE)
            return;
        ...
        for (let i = 0; i < activeCount; i++) {
            ...
            const outOfBounds = particle.position.x < this.bounds.minX ||
            ...
            if (this.currentWeather === WeatherType.RAIN) {
                ...
                this.dummy.lookAt(particle.position.x + particle.velocity.x, particle.position.y + particle.velocity.y, particle.position.z + particle.velocity.z);
            }
            else if (this.currentWeather === WeatherType.ASH) {
                // Ash faces camera
    ...
    static getWeatherForTheme(theme) {
        switch (theme) {
            case LevelTheme.ICE:
    ...
    dispose() {
        ...
        this.fireGlowMesh.material.dispose();
    }
}
